/**
 * Table header configurations for various data entities.
 * These are used by the DataTable component to map row data to specific UI columns.
 */

export const PATIENT_TABLE_HEADERS = [
    { name: 'Patient Name', key: 'name' },
    { name: 'Phone', key: 'phoneNumber' },
    { name: 'Sex', key: 'sex' },
    { name: 'Age', key: 'dob' },
    { name: 'Status', key: 'patientStatus' },
    { name: 'Risk Level', key: 'riskLevel' },
    { name: 'Triage Level', key: 'triageLevel' },
    { name: 'Diseases', key: 'diseases' }
];

export const DOCTOR_TABLE_HEADERS = [
    { name: 'Doctor Name', key: 'name' },
    { name: 'Email', key: 'email' },
    { name: 'Phone', key: 'phoneNumber' },
    { name: 'Sex', key: 'sex' },
    { name: 'Org Name', key: 'orgName' },
];

export const NURSES_TABLE_HEADERS = [
    { name: 'Nurse Name', key: 'name' },
    { name: 'Email', key: 'email' },
    { name: 'Phone', key: 'phoneNumber' },
    { name: 'Sex', key: 'sex' },
    { name: 'Org Name', key: 'orgName' },
];

export const ASHA_TABLE_HEADERS = [
    { name: 'Asha Name', key: 'name' },
    { name: 'Email', key: 'email' },
    { name: 'Phone', key: 'phoneNumber' },
    { name: 'Sex', key: 'sex' },
    { name: 'Org Name', key: 'orgName' },
];

export const HOSPITAL_TABLE_HEADERS = [
    { name: 'Hospital Name', key: 'name' },
    { name: 'Contact Number', key: 'contactNumber' },
    { name: 'Address', key: 'address' },
];

export const REMOVED_PATIENT_TABLE_HEADERS = [
    { name: 'Patient Name', key: 'name' },
    { name: 'Contact Number', key: 'contactNumber' },
    { name: 'Address', key: 'address' },
    { name: 'Reason of Removal', key: 'deletionReason' },
];